...
void spawnLeafling() {
	...
	Material::Sptr leaflingMaterial = ResourceManager::CreateAsset<Material>(basicShader);
	{
		leaflingMaterial->Name = "Leafling";
		leaflingMaterial->Set("u_Material.Diffuse", leaflingTex);
		leaflingMaterial->Set("u_Material.Shininess", 0.1f);
	}
	GameObject::Sptr Leafling = scene->CreateGameObject("Leafling");
	{
		Leafling->SetPostion(scene->MainCamera->GetGameObject()->GetPosition() + glm::vec3(0, 0, -8.0f));
		...
		enemyBehaviour->startPos = scene->MainCamera->GetGameObject()->GetPosition() + glm::vec3(0, 0, -8.0f);
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (!isGamePaused)
		{//doesn't update components if its paused
			scene->Update(dt);
			scene->SetupShaderAndLights(); //Recalculates lighting every frame
		}
