...
void main() {
	...
	//Getting the speclar texture
	vec4 specularColor = texture(u_Material.Specular, inUV);
	...
	vec3 lightAccumulation = CalcAllLightContribution(inWorldPos, normal, u_CamPos, specularColor.x);

	// combine for the final result
	vec3 result = lightAccumulation  * inColor * textureColor.rgb;

	frag_color = vec4(mix(result, reflected, specularColor.x), textureColor.a);
}