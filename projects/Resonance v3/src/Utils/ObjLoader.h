...
static std::map <std::string, MeshBuilder<VertexType>> meshMap;
...
public:
	template <typename VertexType = VertexPosNormTexColTangents>
	static VertexArrayObject::Sptr LoadFromFile(const std::string& filename, bool calcTangents = true, bool isasync = false);
...
VertexArrayObject::Sptr ObjLoader::LoadFromFile(const std::string& filename, bool calcTangents, bool isasync) {
	...
	if (!isasync) {
		...
		if (meshMap<VertexType>.find(filename) != meshMap<VertexType>.end()) {
			isMeshBuilt = true;
		}
	}
	...
	if (!isasync) {
		...
		//if (!(meshMap<VertexType>.find(filename) != meshMap<VertexType>.end()))
			meshMap<VertexType>.insert({ filename, mesh });
