...
void PostProcessingLayer::OnAppLoad(const nlohmann::json & config)
{
	...
	GetEffect<ColorCorrectionEffect>()->Lut = ResourceManager::CreateAsset<Texture3D>("luts/cool.cube");
	_effects[0]->Enabled = false;

	_effects.push_back(std::make_shared<ColorCorrectionEffect>());
	GetEffect<ColorCorrectionEffect>()->Lut = ResourceManager::CreateAsset<Texture3D>("luts/warm.CUBE");
	_effects[1]->Enabled = false;
